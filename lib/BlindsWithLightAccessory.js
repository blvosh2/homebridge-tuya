...
class SimpleBlindsAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        const light_service = this.accessory.getService(Service.Lightbulb);
        this._checkServiceName(service, this.device.context.name);
        this.dpAction = '1';
        this.dpPower = '101';
        console.log(this.device.context.cmdOpen)
        ...
        const characteristicOn = light_service.getCharacteristic(Characteristic.On)
        ...
        this.device.on('change', changes => {
            if (changes.hasOwnProperty(this.dpPower) && characteristicOn.value !== changes[this.dpPower]) {
                ...
                console.log("[Tuya] Light saw change to " + changes[this.dpPower]);
            }
